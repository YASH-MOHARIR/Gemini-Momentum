      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1 text-xs text-slate-500">
            ...
            {entry.action === 'skipped' && !entry.matchedRule && <span>No matching rule</span>}
          </div>
        </div>
...
export default function AgentActivity({ onStop, onPause, onEditRules }: Props) {
  ...
  const getDisplayFolderName = () => {
    if (!config?.watchFolders || config.watchFolders.length === 0) return 'No folders'
    if (config.watchFolders.length === 1) {
      return config.watchFolders[0].split(/[/\\]/).pop() || 'Unknown'
    }
    return `${config.watchFolders.length} folders`
  }

  const getFullFolderPath = () => {
    if (!config?.watchFolders) return ''
    return config.watchFolders.join('\n')
  }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b border-slate-700 bg-emerald-900/10">
        <div className="flex items-center gap-2 mb-3">
          <div
            className={`w-2.5 h-2.5 rounded-full ${isPaused ? 'bg-amber-400' : 'bg-emerald-400 status-dot-pulse'}`}
          />
        ...
        <div className="flex items-center gap-2 text-sm text-slate-300 mb-2">
          ...
          <span className="truncate" title={getFullFolderPath()}>
            Watching: <span className="text-slate-100">{getDisplayFolderName()}</span>
          </span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          <div className="bg-slate-800/50 rounded-lg px-2 py-1.5">
            ...
            <div className="text-sm font-medium text-slate-200">{formatDuration(duration)}</div>
          </div>
          ...
          <div className="bg-slate-800/50 rounded-lg px-2 py-1.5">
            ...
            <div className="text-sm font-medium text-slate-200">{stats.filesProcessed}</div>
          </div>
          ...
          <div className="bg-slate-800/50 rounded-lg px-2 py-1.5">
            ...
            <div className="text-sm font-medium text-slate-200">{stats.aiCalls}</div>
          </div>
        </div>
        ...
        <div className="p-3">
          <div className="flex items-center gap-2 mb-3">
            ...
            {recentActivity.length > 0 && (
              <span className="text-xs text-slate-600">({recentActivity.length})</span>
            )}
          </div>
          ...
          {recentActivity.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-slate-500">Waiting for new files...</p>
              <p className="text-xs text-slate-600 mt-1">Drop files in the watch folder</p>
            </div>
          ) : (
      ...
      <div className="p-3 border-t border-slate-700 bg-slate-800/50">
        <div className="flex gap-2">
          <button
            onClick={onEditRules}
            className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/20 text-sm rounded-lg transition-colors"
