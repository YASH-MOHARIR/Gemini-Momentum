...
export default function StoragePanel() {
    ...
    return (
      <div className="p-4 flex flex-col items-center justify-center h-full text-center">
        ...
        <p className="text-sm text-slate-500 max-w-xs">
          Ask Momentum to analyze storage in a folder
        </p>
    ...
    try {
      ...
      if (largestFiles.length > 0) {
        ...
        for (const file of largestFiles.slice(0, 20)) {
          ...
          lines.push(
            `"${file.name}",${formatBytes(file.size)},${file.type || 'Unknown'},${modified},"${file.path}"`
          )
        }
        lines.push('')
      }
      ...
      if (oldFiles && oldFiles.length > 0) {
        ...
        for (const file of oldFiles) {
          const modified = file.modified ? formatDate(file.modified) : 'N/A'
          lines.push(
            `"${file.name}",${formatBytes(file.size)},${file.age || 'N/A'},${modified},"${file.path}"`
          )
        }
        lines.push('')
      }
  ...
  return (
    <div className="p-3 space-y-4 overflow-y-auto">
      ...
      {actionResult && (
        <div
          className={`flex items-center gap-2 p-2 rounded-lg text-xs ${actionResult.type === 'success' ? 'bg-emerald-900/30 border border-emerald-700/50 text-emerald-300' : 'bg-red-900/30 border border-red-700/50 text-red-300'}`}
        >
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="text-slate-400 border-b border-slate-600">
                  <th
                    className="text-left py-1 px-1 cursor-pointer hover:text-slate-200"
                    onClick={() => handleSort('name')}
                  >
                    Name {sortConfig.key === 'name' && <ArrowUpDown className="w-3 h-3 inline" />}
                  </th>
                  <th
                    className="text-right py-1 px-1 cursor-pointer hover:text-slate-200"
              ...
              <tbody>
                {sortedFiles.slice(0, 10).map((file, i) => (
                  <tr key={i} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                    <td
                      className="py-1 px-1 text-slate-300 truncate max-w-[120px]"
                      title={file.path}
                    >
                      {file.name}
                    </td>
          ...
          {showOldFiles && (
            <div className="mt-2 space-y-1">
              {oldFiles.slice(0, 10).map((file, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between text-xs py-1 px-1 bg-slate-800/50 rounded"
                >
